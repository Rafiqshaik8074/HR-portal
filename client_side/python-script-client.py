...
def push_image_to_dockerhub(image_name, new_image_tag):
    try:
        ...
        subprocess.run(f"docker tag {image_name}:{new_image_tag} sakthibazz/{image_name}:{new_image_tag}", shell=True, check=True)
        client.images.push(f"sakthibazz/{image_name}", tag=new_image_tag)
...
def delete_older_images(image_name, new_image_tag, image_tag):
    try:
        ...
        client.images.remove(f"sakthibazz/{image_name}:{new_image_tag}")
...
# Main function
def main():
    container_name = "hrportal-client"
